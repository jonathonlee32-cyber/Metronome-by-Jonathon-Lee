...
class MetronomeViewModel(application: Application) : AndroidViewModel(application) {
    ...
    /**
     * 如果节拍器正在响就把它停到"暂停"（需求九：开始播放录音之前先暂停节拍器）。
     *
     * 走的是和「暂停」按钮完全相同的那一条路：结算这一段练习、停引擎、秒表停在原地。
     * 所以行为上没有任何新花样 —— 用户从录音页回到节拍器时，看到的就是"刚才被暂停了"，
     * 按一下「开始」就接着响，秒表和练习记录也接着走。
     *
     * 已经在暂停状态时什么都不做（不重复结算、不清零）；播放录音**结束**时也不会自动恢复
     * 节拍器（需求九），所以这里只有"暂停"这一个方向。
     */
    fun pauseIfPlaying() {
        if (uiState.isPlaying) togglePlay()
    }
